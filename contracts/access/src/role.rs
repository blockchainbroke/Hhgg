//! Role type definitions and helpers.
//!
...
/// The three protocol roles. Admin and Council are privileged; Player
/// is the base role for ordinary users.
...
impl RoleEntry {
    /// Construct a new `RoleEntry` for the given account and role.
...
mod tests {
    ...
    fn test_role_from_str() {
        ...
        assert_eq!(Role::from_str("Random"), None);
    }

    #[test]
    fn test_role_entry_new() {
        let account: AccountId = "alice.testnet".parse().unwrap();
        let entry = RoleEntry::new(account.clone(), Role::Admin, 42);
