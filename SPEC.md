...
- `MAX_BOOST_BPS = 30_000` (i.e. 3x cap, 30% per year max)
...
- **No live cross-contract reads during combat resolution** — combat reads only from `snapshot`
...
- **Owner transfer**: Restricted to Admin role, two-step (propose + accept)
...
- **Saturating math**: All arithmetic uses `u128` + `saturating_*` / `checked_*`
